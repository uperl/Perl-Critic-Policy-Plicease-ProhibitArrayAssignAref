...
of Perl 5.8.x and earlier, since they were unprototyped.  And the default
code generated by C<h2xs> (as of Perl 5.10.0) is autoloaded unprototyped
constants, so any module using the bare output of that suffers.
...
rarely anyway, but as always if you want you can disable it altogether in
...
even if it's more likely to be a simple mis-ordering (which C<use strict>
picks up anyway).
